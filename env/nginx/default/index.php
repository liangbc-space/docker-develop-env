...
if ($_POST['act'] == 'MySQL检测') {
    ...
    $mailRe .= (false !== @mail($_POST["mailAdd"], $mailContent,
...
function sys_linux()
{
    ...
    $res['memCachedPercent'] = (floatval($res['memCached']) != 0) ? round($res['memCached'] / $res['memTotal'] * 100,
        2) : 0; //Cached内存使用率
...
function sys_windows()
{
    ...
    $res['win_n'] = $sysinfo[0]['Caption'] . " " . $sysinfo[0]['CSDVersion'] . " 序列号:{$sysinfo[0]['SerialNumber']} 于" . date('Y年m月d日H:i:s',
            strtotime(substr($sysinfo[0]['InstallDate'], 0, 14))) . "安装";
    //UPTIME
...
<div id="page">
    ...
    <table width="100%" cellpadding="3" cellspacing="0">
        ...
        <tr>
            ...
            <td>绝对路径</td>
            <td><?php echo $_SERVER['DOCUMENT_ROOT'] ? str_replace('\\', '/',
                    $_SERVER['DOCUMENT_ROOT']) : str_replace('\\', '/', dirname(__FILE__)); ?></td>
        </tr>
        <tr>
            <td>管理员邮箱</td>
            <td><?php echo $_SERVER['SERVER_ADMIN']; ?></td>
            <td>探针路径</td>
            <td><?php echo str_replace('\\', '/', __FILE__) ? str_replace('\\', '/',
                    __FILE__) : $_SERVER['SCRIPT_FILENAME']; ?></td>
        </tr>
    </table>
    ...
    <? if ("show" == $sysReShow) { ?>
        <table width="100%" cellpadding="3" cellspacing="0" align="center">
            ...
            <tr>
                <td>内存使用状况<br \><br \><a href="https://www.vpser.net/other/linux-vps-ram.html" target="_blank"><font
    ...
    <?php if (false !== ($strs = @file("/proc/net/dev"))) : ?>
        <table width="100%" cellpadding="3" cellspacing="0" align="center">
            ...
            <?php for ($i = 2; $i < count($strs); $i++) : ?>
                <?php preg_match_all("/([^\s]+):[\s]{0,}(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)/",
                    $strs[$i], $info); ?>
                <tr>
    ...
    <table width="100%" cellpadding="3" cellspacing="0" align="center">
        ...
        <tr>
            ...
            <td width="18%">
                ...
                $disFuns = get_cfg_var("disable_functions");
                ?>
                <?php echo (0 !== preg_match("/phpinfo/i",
                        $disFuns)) ? '<font color="red">×</font>' : "<a href='$phpSelf?act=phpinfo' target='_blank'>PHPINFO</a>"; ?>
            </td>
